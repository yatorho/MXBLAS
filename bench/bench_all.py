...
from mxblas.utils import (
    ...
    calc_diff,
    check_nan_inf,
    gaussian_with_channel_outlier,
)


def gen_problem(M, N, K, SM, SN, SK):
    a_bf16 = gaussian_with_channel_outlier((M, K), dim=0, outlier_ratio=0.003, o_std=10)
    ...
    **kwargs,
    ...
    if method == Method.TE_CUBLAS:
        ...
        if method == Method.COAT:
            func = lambda: kernels.coat_gemm_nt(sp, *args, **kwargs)

        elif method == Method.SG_LANG:
            func = lambda: kernels.sgl_gemm_nt(sp, *args, **kwargs)

        elif method == Method.DEEP_GEMM:
            func = lambda: kernels.deepgemm_gemm_nt(sp, *args, **kwargs)

        elif method == Method.FB_GEMM:
            func = lambda: kernels.fbgemm_gemm_nt(sp, *args, **kwargs)
...
if __name__ == "__main__":
    ...
    def parse_shapes(s):
        return list(map(int, _comma_separated_list(s)))
